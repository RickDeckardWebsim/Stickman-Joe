...
import { Grenade, DebugWeapon, ProceduralThrowable } from './weapon.js';
...
function manageCopSpawning_old(canvas) {
    const now = Date.now();
    if (now < lastCopSpawnTime + copSpawnCooldown) return;

    const playerWanted = Math.floor(world.wantedLevel);
    const zombieWanted = Math.floor(world.zombieWantedLevel);
    if (playerWanted === 0 && zombieWanted === 0) return;

    // How many cops should be active at each wanted level.
    const copsPerWantedLevel = settings.copsPerWantedLevel;
    const targetCopsForPlayer = copsPerWantedLevel[playerWanted];
    const targetCopsForZombies = ZOMBIE_COPS_PER_WANTED_LEVEL[zombieWanted];
    const totalTargetCops = targetCopsForPlayer + targetCopsForZombies;

    const currentCops = enemies.filter(e => e.isCop).length;

    if (currentCops < totalTargetCops) {
        // Spawn a new squad to reinforce.
        const squadSize = Math.min(totalTargetCops - currentCops, Math.floor(Math.random() * 3) + 2); // 2-4 cops per squad
        
        // If the zombie threat is the primary reason for spawning, alert the squad.
        const alertToZombies = targetCopsForZombies > targetCopsForPlayer;
        spawnCopSquad(squadSize, canvas, alertToZombies);
        lastCopSpawnTime = now;
    }
}

function spawnCopSquad(size, canvas, alertToZombies = false) {
    const spawnPoint = getOffscreenSpawnPoint(canvas);
    const squad = [];
    const wantedLevel = Math.floor(world.wantedLevel);

    for (let i = 0; i < size; i++) {
        // Spawn them near each other
        const x = spawnPoint.x + (Math.random() - 0.5) * 50;
        const y = spawnPoint.y + (Math.random() - 0.5) * 50;
        
        let cop;
        // Decide whether to spawn a regular cop or SWAT
        if (wantedLevel >= 3) {
            const swatChance = (wantedLevel - 2) * 0.4; // LVL 3: 40%, LVL 4: 80%, LVL 5: 120% (guaranteed > 1.0)
            if (Math.random() < swatChance) {
                cop = new Swat(x, y);
            } else {
                cop = new Cop(x, y);
            }
        ...
        if (alertToZombies) {
            cop.knowsZombiesAreHostile = true;
        }
