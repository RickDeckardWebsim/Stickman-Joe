...
export class DebugWeapon extends Weapon {
    constructor(owner) {
        super(owner);
        this.name = 'Debug Spawner';
        this.icon = './rifle_icon.png'; // Reuse rifle icon for now
        this.width = 40;
        this.height = 8;
        this.color = '#ff00ff'; // Bright magenta to indicate debug tool
        this.fireRate = 100; // Fast firing
        this.gripPoints = { backHand: { x: 10, y: 0 }, frontHand: { x: 25, y: 0 } };
        
        // Debug weapon doesn't use ammo
        this.ammo = 999;
        this.magSize = 999;
        this.reserveAmmo = 999;
        
        // Available spawn modes
        this.availableFireModes = [
            'blood_droplet', 'blood_spatter', 'blood_stream', 'blood_mist',
            'tears', 'debris_small', 'debris_large', 'dust', 'explosion',
            'shell_pistol', 'shell_rifle', 'shell_shotgun', 'magazine',
            'grenade_pin', 'fire_particle', 'bounce_spark'
        ];
        this.fireMode = 'blood_droplet';
    ...
    update(input, isMouseOverUI, mouseWorldPos) {
        ...
        // Simple firing logic for debug weapon
    ...
    fireOneShot(now = Date.now()) {
        // Debug weapon doesn't consume ammo
        const mousePos = this.lastMousePos || { x: this.owner.x, y: this.owner.y };
        
        // Spawn particle based on current fire mode
        this.spawnDebugParticle(mousePos.x, mousePos.y);
        
        // Visual feedback
        this.recoil = 2;
        // No sound for debug weapon to avoid spam
    }

    spawnDebugParticle(x, y) {
        import('./gore.js').then(goreModule => {
            import('./visual-effects.js').then(effectsModule => {
                import('./shell.js').then(shellModule => {
                    import('./impact.js').then(impactModule => {
                        const { BloodParticle, PointBloodEmitter } = goreModule;
                        const { TearParticle, Explosion } = effectsModule;
                        const { Shell, Magazine, GrenadePin } = shellModule;
                        ...
                        switch(this.fireMode) {
                            case 'blood_droplet':
                                particles.push(new BloodParticle(x, y, vx, vy, Math.random() * 2 + 1, 'droplet'));
                                break;
                            case 'blood_spatter':
                                particles.push(new BloodParticle(x, y, vx, vy, Math.random() * 2 + 1, 'spatter'));
                                break;
                            case 'blood_stream':
                                particles.push(new BloodParticle(x, y, vx, vy, Math.random() * 3 + 2, 'stream'));
                                break;
                            case 'blood_mist':
                                for (let i = 0; i < 5; i++) {
                                    const mistAngle = Math.random() * Math.PI * 2;
                                    const mistSpeed = Math.random() * 1 + 0.5;
                                    particles.push(new BloodParticle(
                                        x + (Math.random() - 0.5) * 10, 
                                        y + (Math.random() - 0.5) * 10,
                                        Math.cos(mistAngle) * mistSpeed,
                                        Math.sin(mistAngle) * mistSpeed,
                                        Math.random() * 1 + 0.5, 
                                        'mist'
                                    ));
                                }
                                break;
                            case 'tears':
                                particles.push(new TearParticle(x, y, vx * 0.5, vy * 0.5, Math.random() * 1.5 + 1));
                                break;
                            case 'debris_small':
                                particles.push(new DebrisParticle(x, y, vx * 2, vy * 2, Math.random() * 3 + 2));
                                break;
                            case 'debris_large':
                                particles.push(new DebrisParticle(x, y, vx * 1.5, vy * 1.5, Math.random() * 6 + 4));
                                break;
                            case 'dust':
                                for (let i = 0; i < 3; i++) {
                                ...
                                break;
                            case 'explosion':
                                particles.push(new Explosion(x, y, 50 + Math.random() * 100));
                                break;
                            case 'shell_pistol':
                                shells.push(new Shell(x, y, randomAngle, { width: 2, height: 4 }));
                                break;
                            case 'shell_rifle':
                                shells.push(new Shell(x, y, randomAngle, { width: 3, height: 6 }));
                                break;
                            case 'shell_shotgun':
                                shells.push(new Shell(x, y, randomAngle, { width: 4, height: 10, type: 'shotgun' }));
                                break;
                            case 'magazine':
                                shells.push(new Magazine(x, y, randomAngle, { width: 6, height: 20, color: '#333' }));
                                break;
                            case 'grenade_pin':
                                shells.push(new GrenadePin(x, y, randomAngle));
                                break;
                            case 'fire_particle':
                                particles.push(new FireDebugParticle(x, y, vx, vy));
                                break;
                            case 'bounce_spark':
                                particles.push(new BounceSparkParticle(x, y, vx * 2, vy * 2));
                                break;
                        }
                    });
                });
    ...
    draw(ctx) {
        ...
        // Draw debug weapon with distinctive colors
        ctx.fillStyle = this.color;
        ctx.fillRect(gunX, gunY, this.width, this.height);
        
        // Add debug indicator
        ctx.fillStyle = '#00ff00';
        ctx.fillRect(gunX + this.width - 8, gunY + 2, 6, this.height - 4);
    }
}

// Debug particle classes
class DebrisParticle {
    constructor(x, y, vx, vy, size) {
        ...
        this.friction = 0.98;
        this.active = true;
        this.color = `rgb(${150 + Math.random() * 50}, ${100 + Math.random() * 30}, ${80 + Math.random() * 20})`;
    }
